...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'Elementor', 'fabrik' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html( sprintf( __( 'Vamtam Elementor Integration (%s)', 'fabrik' ), VAMTAM_THEME_SLUG ) ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'fabrik' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'fabrik' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'fabrik' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'fabrik' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'fabrik' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'WP Super Cache', 'fabrik' ),
			'slug'     => 'wp-super-cache',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'fabrik' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'fabrik' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'fabrik' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'fabrik' ),
		'description' => esc_html__( 'Description', 'fabrik' ),
		'status'      => esc_html__( 'Status', 'fabrik' ),
		'version'     => esc_html__( 'Version', 'fabrik' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'fabrik' );
	}
	if ( strpos( $plugin['slug'], 'fabrik' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'fabrik' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'fabrik' );
