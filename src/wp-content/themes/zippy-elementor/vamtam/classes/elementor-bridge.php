...
class VamtamElementorBridge {
	...
	public static function add_sticky_header_bg_color_to_elementor_global_settings() {
		...
		$kit->add_repeater_row( 'system_colors', [
			...
			'title' => __( 'Sticky Header Bg Color', 'fabrik' ),
	...
	*/
		...
		if ( $use_el_global_defaults ) {
			...
			for ( $i = 1; $i <= 8; $i++ ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( 'Accent %s', 'fabrik' ), $i ),
	...
	protected static function add_theme_fonts_to_elementor_global_fonts() {
		...
		if ( $use_el_global_defaults ) {
			...
			// Adding from vamtam_get_option().
			$our_fonts = [
				'primary-font' => __( 'Primary Font', 'fabrik' ),
				'h1'           => __( 'H1', 'fabrik' ),
				'h2'           => __( 'H2', 'fabrik' ),
				'h3'           => __( 'H3', 'fabrik' ),
				'h4'           => __( 'H4', 'fabrik' ),
			...
			foreach ( $our_fonts as $font_prefix => $font_title ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( '%s', 'fabrik' ), $font_title ),
	...
	public static function register_locations( $elementor_theme_manager ) {
		...
		$elementor_theme_manager->register_location(
			'page-title-location',
			[
				'label' => esc_html__( 'Page Title', 'fabrik' ),
				'multiple' => true,
	...
	public static function get_widget_mods_list() {
		...
		*/
		...
		$widget_mods_list = [
			...
			'button'                                     => [ 'label' => __( 'Button', 'fabrik' ) ],
			'form'                                       => [ 'label' => __( 'Form', 'fabrik' ) ],
			'flip-box'                                   => [ 'label' => __( 'Flip Box', 'fabrik' ) ],
			'toggle'                                     => [ 'label' => __( 'Toggle', 'fabrik' ) ],
			'nav-menu'                                   => [ 'label' => __( 'Nav Menu', 'fabrik' ) ],
			'loop-carousel'                              => [ 'label' => __( 'Loop Carousel', 'fabrik' ) ],
			'search-form'                                => [ 'label' => __( 'Search Form', 'fabrik' ) ],
			'section'                                    => [ 'label' => __( 'Section', 'fabrik' ) ],
			'archive-posts'                              => [ 'label' => __( 'Archive Posts', 'fabrik' ) ],
			'posts'                                      => [ 'label' => __( 'Posts', 'fabrik' ) ],
			'post-comments'                              => [ 'label' => __( 'Post Comments', 'fabrik' ) ],
			'popup' /* Not a widget but same logic */    => [ 'label' => __( 'Popup', 'fabrik' ) ],
		];
	...
	*/
	...
	public static function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', esc_html__( 'Could not unzip or empty archive.', 'fabrik' ) );
		}
