...
class VamtamPurchaseHelper extends VamtamAjax {
	...
	private static function server_tests() {
		...
		$tests = array(
			array(
				'name'  => esc_html__( 'PHP Version', 'fabrik' ),
				'test'  => version_compare( phpversion(), '5.5', '<' ),
				'value' => phpversion(),
				'desc'  => esc_html__( 'While this theme works with all PHP versions supported by WordPress Core, PHP versions 5.5 and older are no longer maintained by their developers. Consider switching your server to PHP 5.6 or newer.', 'fabrik' ),
			),
			array(
				'name'  => esc_html__( 'PHP Time Limit', 'fabrik' ),
				'test'  => $timeout > 0 && $timeout < 30,
				'value' => $timeout,
				'desc'  => esc_html__( 'The PHP time limit should be at least 30 seconds. Note that in some configurations your server (Apache/nginx) may have a separate time limit. Please consult with your hosting provider if you get a time out while importing the demo content.', 'fabrik' ),
			),
			array(
				'name'  => esc_html__( 'PHP Memory Limit', 'fabrik' ),
				'test'  => (int) $memory > 0 && $memoryB < 96 * 1024 * 1024,
				'value' => $memory,
				'desc'  => esc_html__( 'You need a minimum of 96MB memory to use the theme and the bundled plugins. For non-US English websites you need a minimum of 128MB in order to accomodate the translation features which are otherwise disabled.', 'fabrik' ),
			),
			array(
				'name'  => esc_html__( 'PHP ZipArchive Extension', 'fabrik' ),
				'test'  => ! class_exists( 'ZipArchive' ),
				'value' => '',
				'desc'  => esc_html__( 'ZipArchive is a requirement for importing the demo sliders.', 'fabrik' ),
			),
		);
	...
	public static function tgmpa_plugins_complete_actions( $update_actions, $plugin_info ) {
		if ( isset( $update_actions['dashboard'] ) ) {
			$update_actions['dashboard'] = sprintf(
				esc_html__( 'All plugins installed and activated successfully. %1$s', 'fabrik' ),
				'<a href="' . esc_url( admin_url( 'admin.php?page=vamtam_theme_setup_import_content' ) ) . '" class="button button-primary">' . esc_html__( 'Continue with theme setup.', 'fabrik' ) . '</a>'
			);
	...
	public static function admin_menu() {
		add_menu_page( esc_html__( 'VamTam', 'fabrik' ), esc_html__( 'VamTam', 'fabrik' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ), '', 2 );
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Dashboard', 'fabrik' ), esc_html__( 'Dashboard', 'fabrik' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ) );
	...
	public static function admin_menu_1() {
		...
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Import Demo Content', 'fabrik' ), esc_html__( 'Import Demo Content', 'fabrik' ), 'edit_theme_options', 'vamtam_theme_setup_import_content', array( __CLASS__, 'vamtam_theme_setup_import_content' ) );
	}

	public static function admin_menu_2() {
		add_submenu_page(
			'vamtam_theme_setup',
			esc_html__( 'Services', 'fabrik' ),
			esc_html__( 'Services', 'fabrik' ) .
		...
		<div class="vamtam-ts-notice">
				<div class="vamtam-services-notice notice cta is-dismissible">
					...
					<div class="vamtam-notice-content">
						<h3><?php echo __( 'Make updates easy with our Premium Service', 'fabrik' ); ?></h3>
						<p><?php echo __( 'Enjoy hassle-free updates with our Premium Services. Keep your software up-to-date effortlessly.', 'fabrik' ); ?></p>
						<p>
	...
	public static function registration_warning() {
		...
		<div class="vamtam-notice-wrap">
			<div class="vamtam-notice">
				<p>
					<?php echo esc_html__( 'Please activate your license to get theme updates, premium support, and access to demo content.', 'fabrik' ); ?>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=vamtam_theme_setup' ) ); ?>">
						<?php echo esc_html__( 'Register Now', 'fabrik' ); ?>
					</a>
				</p>
			</div>
		</div>
	...
	public static function admin_init() {
		...
		add_settings_field(
			...
			esc_html__( 'Enter your purchase code from ThemeForest to receive theme updates and support.', 'fabrik' ),
		...
		<div id="vamtam-ts-homepage" class="vamtam-ts">
			...
			<div id="vamtam-ts-main">
							...
							<form id="vamtam-register-form" method="post" action="options.php" autocomplete="off">
								<?php if ( $valid_key ) : ?>
									<div id="vamtam-verified-code">
										<p>
											<?php  esc_html_e( 'Thanks for the verification!', 'fabrik' ) ?>
											<br />
											<?php echo esc_html( sprintf( __( 'You can now enjoy %s and build great websites.', 'fabrik' ) , $theme_name ) ); ?>
										</p>
									</div>
								<?php else : ?>
				...
				<div id="vamtam-check-license-disclaimer">
					<h5><?php esc_html_e( 'Licensing Terms', 'fabrik' ); ?></h5>
					<p>
						<?php esc_html_e( 'You need to register a separate license for each domain on which you will use the theme. A single license is limited to a single domain/application. For more information, please refer to these articles - ', 'fabrik' ); ?>
						<a href="http://themeforest.net/licenses" target="_blank">
							<?php esc_html_e( 'Licensing Terms Envato Market', 'fabrik' ); ?>
						</a>,
						<a href="https://elements.envato.com/license-terms" target="_blank">
							<?php esc_html_e( 'Licensing Terms Envato Elements', 'fabrik' ); ?>
						</a>
						.
					</p>
				</div>
						...
						<div id="vamtam-server-tests">
							<h3>
								<?php if ( $status['fail'] > 0 ) : ?>
									<?php esc_html_e( 'System Status', 'fabrik' ) ?>
									<?php $fail = $status['fail']; ?>
									<small><?php printf( esc_html( _n( '(%d potential issue)', '(%d potential issues)', $fail, 'fabrik' ) ), $fail ) ?></small>
								<?php endif ?>
							</h3>
						</div>
					<?php endif ?>
	...
	public static function dashboard_navigation() {
		...
		<nav id="vamtam-ts-nav-menu">
			<div id="vamtam-theme-title">
				<span id="vamtam-ts-greeter"><?php esc_html_e( 'WELCOME TO', 'fabrik' ); ?></span>
				<span id="vamtam-ts-greeter-title"><?php echo esc_html( $theme_name ); ?></span>
				<span id="vamtam-ts-greeter-ver"><?php echo sprintf( esc_html__( 'VER. %s', 'fabrik' ), $theme_version ); ?></span>
			</div>
			<ul>
				<li class="<?php echo esc_attr( $cur_route === 'toplevel_page_' . $routes[0] ? 'is-active' : '' ); ?>" >
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[0] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Register' , 'fabrik' ); ?></span>
				...
				<?php if ( isset( $tgmpa_instance ) && isset( $tgmpa_instance->page_hook ) ) : ?>
					<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[1] ? 'is-active' : '' ); ?>" >
						<a <?php echo esc_attr( ! $valid_key ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[1] ) ); ?>">
							...
							<span><?php echo esc_html__( 'Install Plugins' , 'fabrik' ); ?></span>
				...
				<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[2] ? 'is-active' : '' ); ?>" >
					<a <?php echo esc_attr( ! $valid_key || $plugin_status !== 'success' ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[2] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Import Demo' , 'fabrik' ); ?></span>
				...
				<li>
					<a id="vamtam-hs-btn" class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[3] ? 'is-active' : ''); ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[3] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Help & Support' , 'fabrik' ); ?></span>
					</a>
				</li>
				<li>
					<a href="<?php echo esc_url( 'https://vamtam.com/services/' ); ?>" target="_blank">
						...
						<span><?php echo esc_html__( 'Premium Services' , 'fabrik' ); ?></span>
					</a>
				</li>
			</ul>
	...
	public static function import_buttons() {
		...
		$messages = array(
			'success-msg' => esc_html__( 'Imported.', 'fabrik' ),
			'error-msg  ' => esc_html__( 'Failed to import. Please <a href="{fullimport}" target="_blank">click here</a> in order to see the full error message.', 'fabrik' ),
		);

		$import_tests = array(
			array(
				'test'   => defined( 'ELEMENTOR_PRO__FILE__' ),
				'title'  => esc_html__( 'Posts, Pages and Site Layout', 'fabrik' ),
				'failed' => wp_kses( __( "This theme requires Elementor Pro. If you don't have Elementor Pro, please <a href='https://be.elementor.com/visit/?bta=13981&nci=5383' target='_blank'>download it here</a>. Install and activate it, and then proceed with importing the demo content. If you have any issues with the importer please <a href='https://elementor.support.vamtam.com/support/solutions/articles/245218-vamtam-elementor-themes-how-to-install-the-theme-via-the-admin-panel-' target='_blank'>read this article</a> or reach out to us using <a href='https://vamtam.com/contact-us/' target='_blank'>the form on this page</a>.", 'fabrik' ), 'vamtam-a-span' ),
			),
		);
		...
		$buttons = array(
			array(
				'label'          => esc_html__( 'Dummy Content Import', 'fabrik' ),
				'id'             => 'content-import-button',
				'description'    => esc_html__( 'You are advised to use this importer only on new WordPress sites.', 'fabrik' ),
				'button_title'   => esc_html__( 'Import', 'fabrik' ),
				...
				'data'           => array_merge( $messages, [
					...
					'success-msg'      => sprintf( esc_html__( 'Main content imported. Image import progress: <span class="vamtam-image-import-progress">%s</span>.', 'fabrik' ), $img_progress ),
					'fail-msg'         => esc_html__( 'Failed to import. We recommend that you contact your hosting provider for advice, as solving this issue is often specific to each server.', 'fabrik' ),
					'timeout-msg'      => esc_html__( 'Failed to import. This is most likely caused by a timeout. Please contact your hosting provider for advice as to how you can increase the time limit on your server.', 'fabrik' ),
				] ),
				'additional_msg' => $import_disabled_msg . wp_kses( sprintf( __( '<p class="vamtam-description">Please make sure to <a href="%s" target="_blank">backup</a> any existing content that you need as it will be removed by the import procedure (affects Posts, Pages and Menus).</p><p class="vamtam-description">We recommend that you use the <a href="%s" target="_blank">Post Name permalink structure</a></p><p class="vamtam-description">Images will be downloaded in the background after the main import is complete. Depending on your server, this may take several minutes.<br> In the meantime you may notice that some images are not visible.', 'fabrik' ), esc_url( admin_url( 'plugin-install.php?tab=plugin-information&plugin=updraftplus&TB_iframe=true&width=772&height=921' ) ), esc_url( admin_url( 'options-permalink.php' ) ) ), 'vamtam-admin' ),
				'disabled_msg_plain' => '',
			),
		);
	...
	public static function purchase_key( $args ) {
		...
		$placeholder = __( 'XXXXXX-XXX-XXXX-XXXX-XXXXXXXX', 'fabrik' );
		$plugin_status = VamtamPluginManager::get_required_plugins_status();
		$content_imported = ! ! get_option( 'vamtam_last_import_map', false );
		...
		if ( ! defined( 'ENVATO_HOSTED_SITE' ) ) {
			...
			if ( ! $valid_key ) {
				...
				echo '<label for="vamtam-envato-elements-cb">' . __( 'I downloaded the theme from Envato Elements.', 'fabrik' ) . '</label>';
				echo '</div>';

				echo '<p id="vamtam-code-help">' . wp_kses( sprintf( __( ' <a href="%s" target="_blank">Where can I find my Item Purchase Code?</a>', 'fabrik' ), 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ), 'vamtam-a-span' ) . '</p>';

				echo '<p id="vamtam-code-help-elements" class="hidden">';
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Follow this link to generate a new Envato Elements Token.</a>', 'fabrik' ), esc_url( 'https://api.extensions.envato.com/extensions/begin_activation'
				...
				echo '>' . esc_html__( 'Register', 'fabrik' );
				echo '</button>';
				...
				echo esc_html__( 'Continue to required plguins', 'fabrik' );
				echo '</a>';
				...
				echo esc_html__( 'Continue to demo import', 'fabrik' );
				echo '</a>';
			}
